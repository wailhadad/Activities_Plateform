...
export default {
  ...
  methods: {
    async fetchChildren() {
      ...
      try {
const response = await axios.get(`${process.env.VUE_APP_API_URL}:8000/api/show/parent/demande/activity/enfants/${requestId}/${activityId}`);        this.children = response.data;
        this.loading = false;
      } catch (error) {
